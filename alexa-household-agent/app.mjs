...
function run(goal){try{
 ...
 try{save(storage,state);persisted=true;$('saved').textContent=`${state.events.length} simulated steps saved · refresh to resume`;}
 catch{
  $('saved').textContent='Storage unavailable. This preview lasts only in this tab. Refresh may restore an older plan.';
...
$('reset').addEventListener('click',()=>{try{storage.removeItem('household-relay-v1');location.reload()}catch{$('saved').textContent='Could not clear browser storage. The current plan is unchanged.';}});
restore();render();
